...
const TaskComponent = ({ user }) => {
  const [mentees, setMentees] = useState([
    { name: 'Mentee 1', role: 'Mentee', mentor: user.name, tasks: [], resources: [] },
    { name: 'Mentee 2', role: 'Mentee', mentor: user.name, tasks: [], resources: [] },
    { name: 'Mentee 3', role: 'Mentee', mentor: user.name, tasks: [], resources: [] },
  ]);

  const [selectedMentee, setSelectedMentee] = useState(null);
  const [newTask, setNewTask] = useState({ type: '', description: '', dueDate: '' });
  ...
  const handleMenteeSelect = (mentee) => {
    ...
    setNewTask({ type: '', description: '', dueDate: '' });
    ...
    const updatedMentees = mentees.map((mentee) => {
      if (mentee.name === selectedMentee.name) {
        const updatedTasks = editingTaskId
          ...
          : [...mentee.tasks, { ...newTask, id: Date.now(), status: TaskStatus.NOT_COMPLETE }];
  ...
  const handleEditClick = (task) => {
    setNewTask({ type: task.type, description: task.description, dueDate: task.dueDate });
    ...
    const updatedMentees = mentees.map((mentee) => {
      if (mentee.name === selectedMentee.name) {
        const updatedTasks = mentee.tasks.map((task) => {
          if (task.id === taskId) {
            const newStatus = task.status === 'COMPLETE' ? 'NOT_COMPLETE' : 'COMPLETE';
      ...
      if (mentee.name === selectedMentee.name) {
        const updatedResources = editingResourceId
          ? mentee.resources.map((resource) =>
              resource.id === editingResourceId ? { ...resource, ...newResource } : resource
            )
  ...
  const handleDeleteResourceClick = (resourceId) => {
    const updatedMentees = mentees.map((mentee) => {
      if (mentee.name === selectedMentee.name) {
        const updatedResources = mentee.resources.filter(resource => resource.id !== resourceId);
        return { ...mentee, resources: updatedResources };
      }
      return mentee;
    });
  ...
  const currentTasks = filteredTasks.filter((task) => task.status === TaskStatus.NOT_COMPLETE);
  const pastTasks = filteredTasks.filter((task) => task.status === TaskStatus.COMPLETE);
  ...
  <div className="mt-[80px]">
    <div className="flex flex-row space-x-6 p-12 px-4">
      ...
      <div className="flex-none w-1/4 bg-gray-100 rounded-lg shadow-lg">
        ...
        <ul className="bg-white rounded-lg shadow-md p-4">
          {mentees.map((mentee, index) => (
            <li 
              key={index} 
              className={`py-1 cursor-pointer ${selectedMentee?.name === mentee.name ? 'font-bold text-blue-500' : ''}`} 
      ...
      <div className="flex-1 bg-gray-100 rounded-lg shadow-lg p-4 w-2/4">
        ...
        <div className="mt-8">
          <h2 className="text-xl font-semibold text-center mb-2">{editingTaskId ? 'Edit' : 'Add New'} Task</h2>
          <form onSubmit={(e) => { e.preventDefault(); handleAddOrEditTask(); }} className="space-y-4">
            <input
              type="text"
            ...
            />
            <button type="submit" className="w-full bg-[#1F2839] text-white py-2 rounded">
            ...
            <tbody>
              {currentTasks.map((task) => (
                <tr key={task.id} className="border-b">
                  <td className="px-4 py-2 text-center">{task.type}</td>
                  ...
                  <td className="px-4 py-2 text-center">
                    <button onClick={() => handleEditClick(task)} className="text-blue-500 mr-2">Edit</button>
                    <button onClick={() => handleDeleteClick(task.id)} className="text-red-500">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {resources.length > 0 ? (
            <ul className="bg-white rounded-lg shadow-md p-4">
              {resources.map((resource) => (
                <li key={resource.id} className="flex justify-between items-center mt-2">
                  <a href={resource.link} target="_blank" rel="noopener noreferrer" className="text-blue-500">
                    {resource.description}
                  </a>
                  <button onClick={() => handleEditResourceClick(resource)} className="ml-2 text-blue-500">Edit</button>
                  <button onClick={() => handleDeleteResourceClick(resource.id)} className="ml-2 text-red-500">Delete</button>
                </li>
              ))}
            </ul>
          ) : (
        ...
        <div className="flex flex-row space-x-6 p-12 px-4">
          ...
          <div className="flex-none w-1/4 bg-gray-100 rounded-lg shadow-lg p-4">
            ...
            <ul className="bg-white rounded-lg shadow-md p-4">
              {resources.map((resource, index) => (
                <li key={index} className="py-1">
                  {resource.mentor}: 
                  <a href={resource.link} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline ml-1">
                    {resource.description}
                  </a>
                </li>
              ))}
            ...
            {user.role === 'Mentor' && (
              <div className="mt-4">
                <button
                  onClick={() => handleAddResource({ mentor: user.name, description: 'New Resource', link: '#' })}
